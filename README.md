...
- `/v0.5/` — v0.5 인터랙티브 편지
- `/v0.6/` — v0.6 순차 메모 꺼내기
- `/v0.7/` — v0.7 접힌 지도와 여행의 첫 선택
- `/v0.8/` — v0.8 창가에서 보내는 설명 없는 10초
- `/archive/` — 전체 공개 버전
...
마이그레이션 전 공개본과 제작 중간본은 `legacy/`에 원문 그대로 보존합니다. 서비스에서 사용하는 복구본은 각 `v0.x/` 경로에 독립적으로 둡니다.

기획 문서와 버전 기록은 별도의 비공개 저장소 `letsgojeju2026-lab`에서 관리하며, 이 저장소의 로컬 `docs/` 폴더는 Git 추적 대상에서 제외합니다.

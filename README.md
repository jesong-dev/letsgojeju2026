...
2026년 제주 여행을 준비하며 공개한 모바일 브로슈어를 버전별로 보존하는 정적 웹 프로젝트입니다. 현재 공개 버전은 `v0.4`입니다.
...
1. `src/versions/v0.5/`에 `index.ts`, `content.ts`, `style.css`를 추가합니다.
2. 공개 당시 이미지 파일을 `public/images/v0.5/`에 넣습니다.
3. `src/core/version-registry.ts`에 버전 메타데이터를 등록합니다.
4. `v0.5/index.html`을 만들고 `vite.config.ts`의 빌드 입력에 추가합니다.
5. `src/main.ts`에서 해당 버전 렌더러를 연결합니다.
...
현재 작업 폴더는 유효한 Git 저장소가 아니므로 자동 초기화하지 않았습니다.
